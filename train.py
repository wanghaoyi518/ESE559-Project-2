...
# def train(agent, training_envs, num_episodes=2000, batch_size=64, 
#           save_freq=100, log_freq=10, render=False, results_dir='results',
#           additional_envs=0, additional_env_freq=200):
#     """
#     Train the DQN agent on multiple environments with different goals.
    
#     Args:
#         agent (DQNAgent): The DQN agent to train.
#         training_envs (list): List of training environment configurations.
...
#         additional_env_freq (int): Frequency of adding new random environments.
...
#     # If additional random environments are requested, generate them
#     if additional_envs > 0:
#         for _ in range(additional_envs):
#             random_obstacles = RobotEnv.generate_random_environment()
#             random_goal = RobotEnv.sample_random_goal()
...
#     for episode in pbar:
#         # Generate a new random environment occasionally if requested
#         if additional_env_freq > 0 and episode > 0 and episode % additional_env_freq == 0:
#             random_obstacles = RobotEnv.generate_random_environment()
#             random_goal = RobotEnv.sample_random_goal()
#             training_envs.append({
#                 'obstacles': random_obstacles,
#                 'goal': random_goal
#             })
#             print(f"\nAdded new random environment. Total environments: {len(training_envs)}")
        
#         # Randomly select an environment from training environments
#         env_idx = np.random.randint(0, len(training_envs))
#         env_config = training_envs[env_idx]
        
#         # Sample random initial orientation
#         init_phi = np.random.uniform(-np.pi, np.pi)
#         initial_state = (-1.2, -1.2, init_phi)  # Fixed initial position with random orientation
        
#         # Sometimes randomize the goal location within specified regions
#         if np.random.random() < 0.5:  # 50% chance to use a random goal instead of predefined
#             goal = RobotEnv.sample_random_goal()
#         else:
#             goal = env_config['goal']
        
#         # Create environment
#         env = RobotEnv(env_config['obstacles'], goal, initial_state)
...
#         # Run episode
#         while not done and step < 150:  # Max steps per episode as per problem statement
#             # Select action
...
#             agent.remember(state, goal, action, reward, next_state, done)
...
#     parser.add_argument('--additional-envs', type=int, default=2, help='Number of additional random environments')
#     parser.add_argument('--additional-env-freq', type=int, default=200, help='Frequency of adding new environments')
...
#     training_envs = RobotEnv.create_training_environments()
    
#     # Print training setup
#     print(f"Starting training with {args.episodes} episodes")
#     print(f"Initial training environments: {len(training_envs)}")
#     print(f"Will add {args.additional_envs} additional random environments during training")
...
#         additional_env_freq=args.additional_env_freq
#     )
